-- ============================================================================
-- Atomic coupon used_count increment with max_uses guard (Item 2 fix)
-- Same pattern as events.booked_count: atomic UPDATE + ROW_COUNT check
-- ============================================================================
...
BEGIN
  ...
  -- Determine base amount for commission calculation
  ...
  -- Atomic coupon used_count increment with max_uses guard (same pattern as booked_count)
  IF v_payment.coupon_id IS NOT NULL THEN
  ...
  RETURN jsonb_build_object(
    ...
    'discount', v_discount,
    'coupon_updated', v_coupon_updated
  );
END;
$$;
