-- ============================================================================
-- Option A: Atomic coupon claiming at order-creation time
-- claim_coupon  — atomic increment with max_uses guard (lock + check + update)
-- release_coupon — atomic decrement with floor-at-zero guard (used in
--                  reconcile-payments after Razorpay confirms not-paid)
-- confirm_payment — removed the used_count increment (already claimed)
-- ============================================================================

-- ---------------------------------------------------------------------------
-- claim_coupon: called by create-payment-order, atomically reserves a use
-- FOR UPDATE serializes concurrent claim attempts on the same coupon.
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION claim_coupon(p_coupon_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
DECLARE
  ...
  v_updated INTEGER;
BEGIN
  SELECT used_count, max_uses INTO v_used_count, v_max_uses
  FROM public.coupons
...
-- ---------------------------------------------------------------------------
-- release_coupon: called by reconcile-payments after Razorpay confirms an
-- order was never paid. Atomic decrement that never goes below zero.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- confirm_payment — removed the used_count increment (claimed at order time)
-- Also removed v_coupon_updated and the coupon UPDATE block.
-- ---------------------------------------------------------------------------
